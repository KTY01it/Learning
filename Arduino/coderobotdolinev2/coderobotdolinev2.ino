...
//L298 kết nối arduino
  const int motorA1      = 33;  // kết nối chân IN1 với chân 3 arduino
  const int motorA2      = 25;  // kết nối chân IN2 với chân 4 arduino
  const int motorAspeed  = 32;  // kết nối chân ENA với chân 5 arduino
  const int motorB1      = 26; // kết nối chân IN3 với chân 7 arduino
  const int motorB2      = 27; // kết nối chân IN4 với chân 8 arduino
  const int motorBspeed  =14;  // kết nối chân ENB với chân 6 arduino

//kết nối của 3 cảm biến hồng ngoại (dò line )
  const int L_S =12;  // cb dò line phải 
  const int S_S =35;  // cb dò line giữa
  const int R_S =13; //cb dò line trái 
